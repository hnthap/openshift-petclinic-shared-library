// vars/pipelineAppJava.groovy
def call() {
    def label = "slave-${UUID.randomUUID().toString()}"

    def imageNamePostfix = '-thaphn1'

    // Internal infrastructure constants — not operator-facing config
    def secretNexusDocker   = 'nexus-docker-config'
    def secretMavenSettings = 'maven-setting'
    def secretDeploySsh     = 'deploy-ssh-key'
    def pvcMavenCache       = 'jenkins-pvc'
    ...
    podTemplate(
        ...
        volumes: [
            secretVolume(mountPath: '/root/.docker/',        secretName: secretNexusDocker),
            secretVolume(mountPath: '/root/.m2/',            secretName: secretMavenSettings),
            secretVolume(mountPath: '/root/.ssh/',           secretName: secretDeploySsh),
            persistentVolumeClaim(mountPath: '/root/.m2/repository', claimName: pvcMavenCache, readOnly: false)
        ]
    ) {
        node(label) {
            try {
